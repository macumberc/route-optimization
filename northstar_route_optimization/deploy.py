"""Deploy the NorthStar Logistics 3PL Route Optimization demo to Databricks."""
...
""").first()
    ...
    if result["genie_space_id"]:
        print(f"  Genie Space: {result['genie_url']}")
    else:
        print("  Genie Space: (skipped — no warehouse_id or dbutils)")
    print()

    if result["app_url"]:
        print(f"  App: {result['app_url']}")
    elif deploy_app:
        print(f"  App: {workspace_url}/apps/northstar-route-optimization")
    else:
        print("  App: (skipped — deploy_app=False)")
